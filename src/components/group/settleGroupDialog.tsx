...
export function getOptimizedSettlements(
  members: DetailedGroupData["members"],
              ...
              {group.members.map((m: any) => (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span
                      className={`text-[10px] font-bold ${netBalances[m.member.id] >= 0 ? "text-green-500" : "text-orange-500"}`}
                    >
                      {netBalances[m.member.id] >= 0 ? "+" : ""}$
                      {Number(netBalances[m.member.id]).toFixed(2)}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="group relative">
                ...
                <div className="absolute bottom-full right-0 mb-2 w-48 p-2 bg-popover text-[10px] rounded border border-white/10 hidden group-hover:block shadow-xl">
                  Debts are simplified to reduce the total number of payments
                  needed.
                </div>
              </div>
            </div>

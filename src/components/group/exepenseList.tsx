...
export async function ExpenseList({
  ...
  expenses: DetailedGroupData["expenses"];
}) {
        ...
        <Sheet key={expense.id}>
          <SheetTrigger asChild>
            <div className="group grid grid-cols-[auto_1fr_auto] lg:grid-cols-[auto_1fr_120px_150px_120px_100px] items-center gap-4 p-4 rounded-xl border border-white/10 bg-card hover:bg-white/5 transition-all cursor-pointer">
              {/* 1. Date */}
              <div
                className="flex flex-col items-center justify-center w-12 h-12 rounded-lg bg-secondary/50 text-muted-foreground shrink-0"
                suppressHydrationWarning
              >
                <span className="text-[10px] uppercase font-bold">
                  {format(new Date(expense.dateCreated), "MMM")}
                </span>
                <span className="text-lg font-bold leading-none">
                  {format(new Date(expense.dateCreated), "dd")}
                </span>
              </div>
              ...
              <div className="flex flex-col min-w-0">
                <h3
                  className={`font-medium truncate ${!expense.description && "text-muted-foreground italic"}`}
                >
              ...
              <div className="hidden lg:flex flex-col items-center">
                ...
                <div className="flex -space-x-2">
                  {expense.splits.slice(0, 3).map((split: any) => (
                    <div
                      key={split.member.id}
                      className="h-6 w-6 rounded-full border-2 border-card bg-primary/10 flex items-center justify-center text-[10px] font-bold"
                    >
                      {split.member.name[0]}
                    </div>
                  ))}
              ...
              <div className="text-right">
                ...
                <p
                  className={`text-[10px] font-bold uppercase tracking-tight ${
                    expense.whoPaid.id === currentUserr?.id
                      ? "text-green-500"
                      : "text-orange-500"
                  }`}
                >
          ...
          <SheetContent className="w-full sm:max-w-md border-white/10 bg-card text-foreground overflow-y-auto">
              ...
              <div>
                ...
                <div className="space-y-1 bg-white/5 rounded-lg border border-white/5 overflow-hidden">
                  {expense.splits.map((split: any) => (
                    <div
                      key={split.member.id}
                      className="flex items-center justify-between text-sm p-3 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors"
                    >
